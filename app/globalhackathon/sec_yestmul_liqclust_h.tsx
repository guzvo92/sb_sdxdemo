...
export default function Sec_yestmulti_liqclusters_barchart_h({ fullGlobal, categories, metrics, title, suffix = "" }: Props) {

	const [filter, setFilter] = useState<number | null>(null); // null = ALL

	// filtra los tokens según el selector activo
			...
			const datasets = metrics.map((metric, mIndex) => {
				...
				const values = filtered.map(({ data }) => {
					...
					data?.walletblocks?.forEach((wb: any) => {
						...
						// Buscar la metrica en liquiditypercent primero, luego en
						// liquiditypercent_topscluster como fallback. El page del
						// hackathon empaqueta TODAS las percent_* en liquiditypercent.
						let raw: any = last.liquiditypercent?.[metric];
						if (raw === undefined) raw = last.liquiditypercent_topscluster?.[metric];
						if (raw !== undefined) {
	...
	const activeColor = filter === null
		? "#06b6d4"
		: (categories.find(c => c.id === filter)?.color ?? "#06b6d4");
