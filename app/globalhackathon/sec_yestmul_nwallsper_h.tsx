...
// Mapping marino sincronizado con el resto del proyecto.
...
export default function Sec_yestmulti_nwalletspercent_barchart_h({ fullGlobal, categories, metrics, title, suffix = "" }: Props) {
	...
	useEffect(() => {
		let interval = setInterval(() => {
			...
			const datasets = metrics.map((metric, mIndex) => {
				...
				const values = filtered.map(({ data }) => {
					...
					data?.walletblocks?.forEach((wb: any) => {
						...
						if (last.walletblocks_percents && last.walletblocks_percents[metric] !== undefined) {
							const raw = last.walletblocks_percents[metric];
			...
			const canvasId = `globalWalletsPercBarChart${suffix}_${filter ?? "all"}`;
