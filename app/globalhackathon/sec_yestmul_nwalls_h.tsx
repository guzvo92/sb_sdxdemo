...
// Mapping marino sincronizado con el resto del proyecto (sec_datatext_vip2,
// sec_historygrowth, global/page.tsx, sec_token_nwallsclust).
...
// Label pretty para el chart: si el metric es un bracket conocido devuelve
// icono + tier + range; si no, fallback al label generico anterior.
function prettyBracketLabel(metric: string): string {
	const key = metric.replace(/^nholders_/, "").replace(/^percent_nholders_/, "");
	const m = BRACKET_META[key];
	if (m) return `${m.icon} ${m.tier} ${m.range}`;
	return metric.replace(/nholders_/g, "").replace(/percent_/g, "").replace(/_/g, " ").toUpperCase();
}
...
export default function Sec_yestmulti_nwallets_barchart_h({ fullGlobal, categories, metrics, title, suffix = "" }: Props) {
